...
# 웹상으로 전달할 BseModel 기반 클래스 생성
class DetectionResult(BaseModel):
    message: str            # 객체 인식 결과 메시지
    image: str              # 인식 결과 이미지

# 이미지 객체 탐지 함수
def detectObjects(image: Image.Image):
    img = np.array(image)       # Pillow 이미지 numpy 배열로 변환
...
# ASP.NET 에서 전달받은 이미지로 객체 인식, 인식 결과를 다시 ASP.NET으로 전달
@app.post('/detect', response_model=DetectionResult)
async def detectService(message: str = Form(...), file: UploadFile = File(...)):
    # 이미지 읽어서 PIL 이미지로 변환
    image = Image.open(io.BytesIO(awiat file.read()))   # 웹으로 전달된 이미지 객체 로드
